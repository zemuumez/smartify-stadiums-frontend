...
export default function DashboardLayout({
  ...
  return (
    <div className="min-h-screen flex bg-[#f4f3ef] text-[#111] overflow-x-hidden">
      ...
      {/* ── DESKTOP SIDEBAR (Non-scrollable, fits neatly into viewport) ── */}
      <aside
        className={`hidden lg:flex flex-col flex-shrink-0 bg-white border-r border-black/[0.06] sticky top-0 h-screen z-30 transition-all duration-300 ease-in-out overflow-hidden justify-between ${
          collapsed ? "w-[78px]" : "w-[260px]"
        }`}
      >
        {/* Top: Brand Logo */}
        <div>
          <div className="flex items-center gap-3 px-5 h-16 border-b border-black/[0.06] flex-shrink-0">
          ...
          {/* Navigation Items (Tightened spacing, zero vertical scrollbar) */}
          <nav className="py-2.5 px-3 space-y-1 overflow-hidden">
        ...
        {/* Bottom: Sidebar Footer — User & Collapse Button */}
        <div className="p-3 border-t border-black/[0.06] flex-shrink-0 space-y-2">
          {!collapsed && (
            <div className="flex items-center gap-2.5 p-2 rounded-2xl bg-[#f4f3ef]">
              <div
          ...
          <div className="flex items-center justify-between gap-1">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1.5 py-1.5 px-2 rounded-xl text-xs font-bold text-[#5a5a5a] hover:text-[#111] hover:bg-[#f4f3ef] transition-colors"
            ...
            <button
              onClick={() => logout()}
              className="p-1.5 rounded-xl text-[#7a7a7a] hover:text-red-600 hover:bg-red-50 transition-colors"
              title="Sign Out"
            >
              <LogOut size={16} />
            </button>
            ...
            >
              ...
              <div className="flex items-center justify-between px-5 h-16 border-b border-black/[0.06]">
                <div className="flex items-center gap-2.5">
                  ...
                  />
                  <span className="text-sm font-black text-[#111]">Owner Dashboard</span>
                </div>
                <button
                  onClick={() => setMobileOpen(false)}
              ...
              <div className="p-4 border-t border-black/[0.06]">
                <button
                  ...
                  className="flex items-center justify-center gap-2 w-full py-2.5 rounded-full text-xs font-bold text-red-600 bg-red-50 hover:bg-red-100 transition-colors"
                >
      ...
      <div className="flex-1 min-w-0 flex flex-col overflow-y-auto">
        ...
        <header className="bg-white border-b border-black/[0.06] sticky top-0 z-20 shadow-sm">
          <div className="px-4 sm:px-6 lg:px-8 h-16 lg:h-18 flex items-center justify-between gap-4">

            {/* Left: Mobile menu toggle + Page title */}
            <div className="flex items-center gap-3">
              <button
                onClick={() => setMobileOpen(true)}
                className="p-2 rounded-xl border border-black/10 text-[#111] lg:hidden hover:bg-[#f4f3ef]"
              ...
              <div>
                ...
                <h1 className="text-lg sm:text-xl font-black text-[#111] tracking-tight">
                  {getPageTitle()}
                </h1>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              <Link
                href="/microsite"
                target="_blank"
                className="hidden sm:inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-bold text-[#2d6a4f] bg-[#f0faf4] border border-[#2d6a4f]/20 hover:bg-[#2d6a4f] hover:text-white transition-all shadow-sm"
              >
                <Globe size={13} /> View Live Official Site <ArrowUpRight size={11} />
              </Link>
