...
import { Calendar, MapPin, Star, Play, ArrowRight, Clock, Users, Trophy, Shield, ChevronRight, Zap } from "lucide-react";
...
const upcomingEvents = [
  { title: "Friday Night League", date: "Every Friday", time: "7:00 PM", spots: 8 },
  { title: "Youth Tournament", date: "Aug 30, 2026", time: "2:00 PM", spots: 16 },
  { title: "Corporate Cup", date: "Sep 15, 2026", time: "10:00 AM", spots: 12 },
];

const highlights = [
  { title: "Goal of the Month", player: "Abebe K.", description: "Incredible long-range strike from 35 meters", votes: 342 },
  { title: "Best Save", player: "Dawit M.", description: "Penalty save in the 90th minute", votes: 218 },
  { title: "Skill of the Week", player: "Yonas T.", description: "Solo run beating 4 defenders", votes: 189 },
];

const testimonials = [
  { name: "Abebe K.", text: "Best artificial turf in Addis. The camera system is incredible — we can rewatch every goal!", rating: 5 },
  { name: "Fatuma A.", text: "Clean facilities, fair prices, and the microsite makes it easy to book. Highly recommended.", rating: 5 },
  { name: "Daniel T.", text: "We hold our team practices here weekly. The lighting is perfect for evening sessions.", rating: 4 },
];
    ...
    <div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: [0.22, 1, 0.36, 1] }}>
            ...
            <h1 className="text-5xl sm:text-7xl font-black text-white leading-[0.9] mb-6">
              {DEMO.name.split(" ")[0]}<br/>
              <span className="gradient-text">{DEMO.name.split(" ").slice(1).join(" ")}</span>
            </h1>
            <p className="text-xl text-slate-300 mb-8 max-w-xl">Premium football experience in the heart of {DEMO.city}. 4 fields, AI cameras, and world-class facilities.</p>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.8 }} className="mt-16 grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              { value: DEMO.totalBookings.toLocaleString(), label: "Total Bookings", icon: <Calendar size={18} className="text-green-400" /> },
              { value: DEMO.fields, label: "Fields", icon: <Zap size={18} className="text-green-400" /> },
              { value: DEMO.players.toLocaleString(), label: "Players", icon: <Users size={18} className="text-green-400" /> },
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {upcomingEvents.map((event, i) => (
              <motion.div key={event.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="p-6 rounded-2xl bg-slate-50 border border-slate-100 hover:border-green-200 hover:shadow-lg transition-all">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm text-green-600 font-medium">{event.spots} spots left</span>
      ...
      <section className="py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {highlights.map((h, i) => (
              <motion.div key={h.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="rounded-2xl overflow-hidden bg-white border border-slate-200 hover:shadow-lg transition-shadow">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {testimonials.map((t, i) => (
              <motion.div key={t.name} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="p-6 rounded-2xl bg-slate-50 border border-slate-100">
